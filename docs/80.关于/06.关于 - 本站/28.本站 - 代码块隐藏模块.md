...
本内容重新实现的一键复制功能是基于 `vuepress-plugin-one-click-copy` 插件（箭头左边），该插件已经内置 `vuepress-theme-vdoing` 主题，所以无需担心，如果你曾经卸载了该插件，
则需要安装回来；如果已经安装，则无需看这一步：
...
图标库来自阿里云：[https://www.iconfont.cn/](https://www.iconfont.cn/)。
...
在 `docs/.vuepress/config.js`的 plugins 中添加插件配置。
