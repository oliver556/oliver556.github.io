...
regeneratorRuntime 相关的代码我们在 [《ES6 系列之 Babel 将 Generator 编译成了什么样子》](12.ES6系列之Babel将Generator编译成了什么样子.md) 中已经介绍过了，这次我们重点来看看 _asyncToGenerator 函数：
...
当我们执行 `fetchValue()` 的时候，执行的其实就是 _`asyncToGenerator` 返回的这个匿名函数，在匿名函数中，我们执行了
...
从而使得 Generator 继续执行，直到 `info.done` 为 true，才会 `resolve(value)`。
