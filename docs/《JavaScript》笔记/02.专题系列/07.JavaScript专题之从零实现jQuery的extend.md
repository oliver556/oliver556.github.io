...
结合着上篇写得[《JavaScript 专题之深浅拷贝》](06.avaScript专题之深浅拷贝.md)，我们尝试着自己写一个 extend 函数：
...
在实现上，核心的部分还是跟上篇实现的深浅拷贝函数一致，如果要复制的对象的属性值是一个对象，就递归调用 extend。不过 extend 的实现中，多了很多细节上的判断，比如第一个参数是否是布尔值，target 是否是一个对象，
不传参数时的默认值等。
...
结合着 [《JavaScript 专题之类型判断(下)》](05.JavaScript专题之类型判断%28下%29.md) 中的 isPlainObject 函数，我们可以对类型进行更细致的划分：
...
![javascript_03-15_02](https://cdn.jsdelivr.net/gh/oliver556/image-hosting@master/20220315/javascript_03-15_02.3di2z5ggz0m0.webp)
